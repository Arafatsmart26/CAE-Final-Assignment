def Reader(filename):
    ...
    with open(filename, 'rb') as f:
        while True: # keep extracting chunks as long as there are any - we don't know how many instructions there are beforehand, afte rall
            chunk = f.read(size) # Each chunk is 4 bytes
            if not chunk: # This checks if the chunk is empty, if it is we break the while loop
                break
            chunk = chunk[::-1] # flip the chunk because we read the file like it's left to right, while RISC-V isntructions are right to left
