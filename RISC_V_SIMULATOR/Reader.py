...
def Reader():
    ...
    with open('RISC_V_SIMULATOR/tests/task1/addneg.bin', 'rb') as f:
        while True:
            ...
            instructions.append(bin(binToInt(chunk.hex(),scale))[2:].zfill(num_of_bits)) #We convert the hexcode to binary, remove the prefix and fills in extra bits if needed
    return instructions
for i in Reader():
    print(i)
print(len(Reader()))
...
for i in getRegisters():
    print("Register_" + str(j) + " " + hex(int(i.getContents(),2)))
    j += 1
